...
static INJECTION_PATTERNS: &[(&str, &str, InjectionSeverity)] = &[
    (
        ...
        r"(?i)ignore\s+(all\s+)?(previous|above|prior)\s+(instructions|directives|commands|rules|prompts)",
        InjectionSeverity::Critical,
    ),
    (
        "instruction_forget",
        r"(?i)forget\s+(all\s+)?(previous|above|prior)\s+(instructions|directives|commands|rules|prompts)",
        InjectionSeverity::Critical,
    ),
    (
        "instruction_disregard",
        r"(?i)disregard\s+(all\s+)?(previous|above|prior)\s+(instructions|directives|commands|rules|prompts)",
        InjectionSeverity::Critical,
    ),
    ...
    (
        "system_prompt_leak",
        r"(?i)(reveal|show|print|output|display|dump)\s+(your\s+)?(system|instructions|prompt|rules|directives)",
        InjectionSeverity::Critical,
    ),
    (
        "repeat_after",
        r"(?i)(repeat|say|write)\s+(after\s+me|the\s+words|exactly\s+this)",
        InjectionSeverity::High,
    ),
    ("ignore_above", r"(?i)ignore\s+the\s+above", InjectionSeverity::High),
    (
        "delimiter_confusion",
        r"(?i)(<\|im_start\|>|<\|im_end\|>|<\|sys\|>|<\|user\|>|<\|assistant\|>)",
        InjectionSeverity::Critical,
    ),
    (
        "tool_injection",
        r"(?i)use\s+(tool|function)\s+\w+\s+(to|with|on)",
        InjectionSeverity::High,
    ),
    (
        "data_exfiltration",
        r"(?i)(exfiltrate|exfiltrat|leak|steal)\s+(data|info|information|records|customers?|users?)",
        InjectionSeverity::Critical,
    ),
    (
        "mass_export",
        r"(?i)(export|download|copy|transfer|send)\s+(all|every|entire|mass)",
        InjectionSeverity::High,
    ),
    (
        "instruction_redirect",
        r"(?i)(now|instead),?\s+(say|respond|output|answer|write)",
        InjectionSeverity::High,
    ),
    (
        "output_control",
        r"(?i)output\s+\d+\s*(tokens?|words?|chars?)",
        InjectionSeverity::Medium,
    ),
    (
        "hidden_text",
        r"(?i)(<div[^>]*style=[^>]*display:\s*none|<span[^>]*style=[^>]*display:\s*none|<!--.*-->)",
        InjectionSeverity::Critical,
    ),
    (
        "base64_injection",
